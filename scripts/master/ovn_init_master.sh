...
grep -E "kubeadm join|discovery-token" $MAIN_DIR/kubeadm.log | sudo tee $MAIN_DIR/kubeadm_join_tmp
sudo cat $MAIN_DIR/kubeadm_join_tmp |tr '\n\\ ' ' '|sudo tee $MAIN_DIR/kubeadm_join
echo -e "${green}[DONE]${none}"
